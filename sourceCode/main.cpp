...
int main(int argc, char* argv[])
{
    ...
    po::options_description_easy_init opt = desc.add_options();
    opt("headless,h", "runs CoppeliaSim in an emulated headless mode: this simply suppresses all GUI elements (e.g. doesn't open the main window, etc.), but otherwise runs normally");
    opt("true-headless,H", "runs CoppeliaSim in true headless mode (i.e. without any GUI or GUI dependencies). A display server is however still required. Instead of using library coppeliaSim, library coppeliaSimHeadless will be used");
    opt("auto-start,s", po::value<int>(), "automatically start the simulation. If an argument is specified, simulation will automatically stop after the specified amount of milliseconds.");
    opt("auto-quit,q", "automatically quits CoppeliaSim after the first simulation run ended.");
    opt("cmd,c", po::value<std::string>(), "executes the specified script string as soon as the sandbox script is initialized.");
    opt("verbosity,v", po::value<std::string>()->default_value("loadinfos"), "sets the verbosity level, in the console. Default is loadinfos. Accepted values are: none, errors, warnings, loadinfos, scripterrors, scriptwarnings, scriptinfos, infos, debug, trace, tracelua and traceall.");
    opt("statusbar-verbosity,w", po::value<std::string>()->default_value("scriptinfos"), "sets the verbosity level, in the statusbar. Default is scriptinfos. Accepted values are: none, errors, warnings, loadinfos, scripterrors, scriptwarnings, scriptinfos, infos, debug, trace, tracelua and traceall.");
    opt("dialogs-verbosity,x", po::value<std::string>()->default_value("infos"), "sets the verbosity level, for simple dialogs. Default is infos. Accepted values are: none, errors, warnings and questions.");
    opt("addon,a", po::value<std::string>(), "loads and runs an additional add-on specified via its filename.");
    opt("addon2,b", po::value<std::string>(), "loads and runs an additional add-on specified via its filename.");
    opt("param,G", po::value<std::vector<std::string>>(), "sets a named param YYY=XXX: named parameter: YYY represents the key, XXX the value, that can be queried within CoppeliaSim with sim.getNamedStringParam.");
    opt("arg,g", po::value<std::vector<std::string>>(), "sets an optional argument that can be queried within CoppeliaSim with the sim.stringparam_app_arg1... sim.stringparam_app_arg9 parameters. The argument can be used for various custom purposes.");
    opt("options,O", po::value<int>(), "options for the GUI.");
    opt("scene-or-model-file,f", po::value<std::vector<std::string>>(), "input file");
    opt("help", "display command line usage");
    po::options_description desc_internal;
    po::options_description_easy_init internal_opt = desc_internal.add_options();
    internal_opt("devmode,D", po::value<std::string>()->implicit_value("true"), "set developer mode");
